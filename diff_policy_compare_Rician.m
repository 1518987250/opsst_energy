function [ ] = diff_policy_compare_Rician(M,Dm,t,T,c,Ep,str)
...
while(Duration(1)<TotalTime)  
    ...
    while(i<=ob)%前面（根号Z-1）轮可以进行速率判断,得到最大的速率  
       rnd_g=sqrt(K/(K+1))+sqrt(1/(K+1))*raylrnd(fc);%随机生成符合Rician分布的增益值
    ...
    if(i==Z)%到达传输延迟，必须传输E:\Program Files (x86)\MATLAB\R2014b\toolbox\stats\stats\raylrnd.m
         rnd_g=sqrt(K/(K+1))+sqrt(1/(K+1))*raylrnd(fc);%随机生成符合Rician分布的增益值
         while(rnd_g>g)%若超出最大值,则重新生成
            rnd_g=sqrt(K/(K+1))+sqrt(1/(K+1))*raylrnd(fc);%随机生成符合Rician分布的增益值
         end   
...
%第二种情况:确定传输策略,即在最后的期限传输
it=1;%模拟的轮次
itLossData=0;%丢失数据的轮次
sumLossData=0;

while(Duration(2)<TotalTime)
    if(RemainEn(2)<Z*Ep+P*T)%剩余能量不足,生命结束
        break;
    end
    rnd_g=sqrt(K/(K+1))+sqrt(1/(K+1))*raylrnd(fc);%随机生成符合Rician分布的增益值
    while(rnd_g>g)%若超出最大值,则重新生成
    ...
    transdata=TransR*T;%能传输的数据
    if (producedata>transdata)%这段时间内产生的数据更多
       TransData(2)=TransData(2)+transdata;
...
%     end
    ...
    if (producedata>transdata)%这段时间内产生的数据更多
       TransData(3)=TransData(3)+transdata;    %只能传输这么多数据
       formLossData=producedata-transdata;%存在这轮没有传输的数据留到下一轮
    else%这段时间内能产生的数据更多,但没有足够的数据可传输
...
%第三种情况:随机选择一个时间进行传输,结束
%第四种情况:观察37%的时间,记下最大值，如果后面某个速率超过这个最大值，则传输
it=1;%模拟的轮次
itLossData=0;%丢失数据的轮次
sumLossData=0;
formLossData=0;%上一轮没有传输的数据
...
%第四种情况:观察37%的时间,记下最大值，如果后面某个速率超过这个最大值，则传输，结束

%结束后，将所有值存储
name=sprintf('%s%s%s','test_compare_diff_',str,'.txt');
fi=fopen(name,'a');
...
while(i<=numberc)
   fprintf(fi,'Case %d\r\n',i);
   fprintf(fi,'TransData:bit\tDuratiaon:S\tUseEn:J\tEnEf:J/bit\tMaxLossData:bit\tMinLossData\tAveLossData\tAveTransData\tfcLossData\tAveScheduler\tTurnLossData\tTranDataRatio\r\n'); 
   x=(Eninit-RemainEn(i))/TransData(i);
   y=Eninit-RemainEn(i);
   fprintf(fi,'%.0f\t%.0f\t%.3f\t%.15f\t',TransData(i),Duration(i),y,x);
